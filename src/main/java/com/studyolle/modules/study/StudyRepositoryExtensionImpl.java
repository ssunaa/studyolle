...
public class StudyRepositoryExtensionImpl extends QuerydslRepositorySupport implements StudyRepositoryExtension {
    ...
    @Override
    public List<Study> findFirst5ByManagerOrderByPublishedDateTimeDesc(Account account, boolean closed) {
        QStudy study = QStudy.study;
        JPQLQuery<Study> query = from(study).where(study.published.isTrue()
                .and(study.closed.isFalse())
                .and(study.managers.any().in(account)))
                .orderBy(study.publishedDateTime.desc())
                .distinct()
                .limit(5);
        return query.fetch();
    ...
    @Override
    public List<Study> findFirst5ByMemberOrderByPublishedDateTimeDesc(Account account, boolean closed) {
        QStudy study = QStudy.study;
        JPQLQuery<Study> query = from(study).where(study.published.isTrue()
                .and(study.closed.isFalse())
                .and(study.members.any().in(account)))
                .orderBy(study.publishedDateTime.desc())
                .distinct()
                .limit(5);
        return query.fetch();
